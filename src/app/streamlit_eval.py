...
def _render_single_run(st, run_dir: Path):
    ...
    if metrics_csv.exists():
        ...
        if df is not None and len(df) > 0:
            ...
            with colB:
                st.markdown("**Confidence metric over time**")
                st.line_chart(df.set_index("timestamp_sec")[["conf_metric"]])
...
def _render_comparison(st, run_dirs: List[Path]):
    ...
    # Overlay time-series (CRF + live bitrate) across runs.
    ...
    for rd in run_dirs:
        ...
        try:
            ...
            continue
        if len(df) == 0 or "timestamp_sec" not in df.columns:
            continue
        df = df.set_index("timestamp_sec")
